...
public final class JsonUtil {
		...
		if (array != null) {
			...
			for (int i = 0; i < length; i++) {
				...
				if (jsonProcessor != null) {
					subString.append(jsonProcessor.process(null, object, false));
				} else {
		...
		if (object != null) {
			...
			if (fields != null && fields.length > 0) {
				...
				for (int i = 0; i < length; i++) {
					...
					if (jsonProcessor != null) {
						methodReturnValue = jsonProcessor.process(fieldName, methodReturnValue, ignoreFirstLetterCase);
					}
				...
				for (Method method : methods) {
					...
					if (StringUtil.isNotBlank(fieldName)) {
						...
						if (jsonProcessor != null) {
							value = jsonProcessor.process(fieldName, value, ignoreFirstLetterCase);
						}
	...
	public static interface JsonProcessor {
		 ...
		 * @return String
		 */
		public abstract <T extends Object> String process(String fieldName, Object value, boolean ignoreFirstLetterCase);
	}
}
